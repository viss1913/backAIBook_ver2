...
**⚠️ ВАЖНО:** 
- `deviceId` - **обязательное поле** для идентификации пользователя
...
- `tokensRemaining` (number) - Оставшееся количество токенов после генерации
- `cached` (boolean) - `true` если изображение было из кэша (токены не списались)
...
Система токенов контролирует использование генерации изображений:
...
### Endpoints для работы с токенами
...
**📖 Полная документация:** См. `ANDROID_API_COMPLETE.md` для детальных примеров кода и всех endpoints.
